...
test("trusted builtin still uses the sandbox or fails closed",async()=>{const r=await run("print('OK')",{origin:"TRUSTED_BUILTIN"});if(process.platform==='darwin'){if(r.securityReport?.sandboxAvailable)expect(r.stdout.trim()).toBe("OK");else expect(r.blockedReason).toContain("SANDBOX_UNAVAILABLE")}})
