...
test("model code receives no host HOME or API secrets",async()=>{const r=await run("import os, json; print(json.dumps(dict(os.environ)))",{extraEnv:{HOME:"/private",OPENAI_API_KEY:"SECRET",INNOCENT:"SECRET"}});expect(r.stdout).not.toContain("SECRET");expect(r.stdout).not.toContain('HOME')})
...
test("timeout kills the process group",async()=>{const r=await run("while True: pass",{timeoutMs:100});expect(r.timedOut).toBe(true);expect(r.exitCode).toBeNull()})
test("output is bounded in bytes and marked truncated",async()=>{const r=await run("print('x'*100000)",{maxOutputBytes:1024});expect(Buffer.byteLength(r.stdout)).toBeLessThanOrEqual(1024);expect(r.stdoutTruncated).toBe(true)})
test("model code cannot escape its private cwd",async()=>{const outside=join(tmpdir(),`mathos-escaped-${Date.now()}`);const r=await run(`open(${JSON.stringify(outside)},'w').write('BAD')`);expect(r.exitCode).not.toBe(0);await expect(access(outside)).rejects.toThrow()})
...
test("trusted builtin still uses the sandbox",async()=>{const r=await run("print('OK')",{origin:"TRUSTED_BUILTIN"});if(process.platform==='darwin'){expect(r.stdout.trim()).toBe("OK");expect(r.securityReport?.sandboxAvailable).toBe(true)}})
