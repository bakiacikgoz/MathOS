...
describe("product completion baseline", () => {
  test("records exact provenance and fails closed without writing outside the artifact root", async () => {
    ...
    const runner:BaselineCommandRunner=async command=>{
      ...
      if(command.includes("--version")) return {exitCode:0,stdout:"MathOS 1.0.0-rc.1\n",stderr:""}
    ...
    expect(result.versions).toEqual({product:"1.0.0-rc.1",schema:30,bridge:1,pluginApi:1,capsule:1,publication:1})
