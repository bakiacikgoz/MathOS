...
describe("standalone runtime layout",()=>{
  test("resolves installation assets relative to executable and user state to platform roots",()=>{
    expect(resolveRuntimeLayout({platform:"linux",home:"/srv/test-home",executablePath:"/opt/mathos/bin/mathos",env:{XDG_CONFIG_HOME:"/cfg",XDG_DATA_HOME:"/data",XDG_CACHE_HOME:"/cache",XDG_STATE_HOME:"/state"}})).toEqual({
  ...
  test("never derives runtime assets from cwd or source directories",()=>{
    const layout=resolveRuntimeLayout({platform:"darwin",home:"/Volumes/TestHome",executablePath:"/Applications/MathOS/bin/mathos",env:{},cwd:"/tmp/source/packages/core"})
