...
export class BridgeClient {
  constructor(private readonly options: { workspaceRoot: string; trusted: boolean; executablePath?: string }) {}
  spawnSpec() { return { command: this.options.executablePath ?? "mathos", args: ["bridge", "stdio"], cwd: this.options.workspaceRoot, env: { MATHOS_BRIDGE: "1", MATHOS_WORKSPACE_TRUST: this.options.trusted ? "trusted" : "untrusted" } } }
