...
export async function runHeadless(argv: string[]): Promise<number> {
  ...
  try {
    ...
    if (command === "provider") {
      ...
      if (action === "test") { const profile = registry.get(id); if (!profile) throw new Error(`MODEL_PROFILE_NOT_FOUND: ${id}`); const descriptor = providerCatalog.get(profile.descriptorId)!; const policy = evaluateProviderPolicy(descriptor.id); if (!policy.allowed) throw new Error(policy.code); if (rest.includes("--live") && descriptor.billingClass === "payg" && !rest.includes("--accept-usage")) throw new Error("LIVE_USAGE_ACCEPTANCE_REQUIRED"); process.stdout.write(`${JSON.stringify({ profile: id, state: "CONFIGURED", liveRequest: rest.includes("--live") ? "NOT_IMPLEMENTED" : "NOT_REQUESTED", billing: descriptor.billingClass, terms: policy.code }, null, 2)}\n`); return 0 }
