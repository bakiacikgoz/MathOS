...
const MODEL_COMMANDS = new Set(["formalize", "prove", "research", "team", "ingest"])

export function requiredModelRoles(command: string, configuredRoles: Record<string, string>): ModelRole[] {
  if (command === "formalize") return ["formalizer", configuredRoles.alignment ? "alignment" : "auditor"]
  if (command === "prove") return ["prover"]
  if (command === "ingest") return ["researcher"]
  return ["planner", "prover"]
}
...
export async function runHeadless(argv: string[]): Promise<number> {
  ...
  try {
    ...
    if (command === "provider") {
      ...
      const registry = new ProviderProfileRegistry(loadModelProfileStore(path).profiles)
      const save = () => { mkdirSync(dirname(path), { recursive: true }); saveModelProfileStore(path, registry.list()) }
      const action = rest[0] ?? "list", id = rest[1]?.startsWith("--") ? undefined : rest[1]
      const json = rest.includes("--json")
      ...
      if (action === "fallback") { const operation = rest[1] ?? "show", role = flag(rest, "--role") ?? (operation === "set" ? rest[2] : undefined); const configPath = join(layout.userConfigRoot, "config.toml"), existing = existsSync(configPath) ? parseMathOSConfig(readFileSync(configPath, "utf8")) : {}; if (operation === "show") { const entries = Object.fromEntries(Object.entries(existing).filter(([key]) => key.startsWith("model.fallback.") && (!role || key.startsWith(`model.fallback.${role}.`)))); process.stdout.write(`${JSON.stringify({ role: role ?? null, fallback: entries }, null, 2)}\n`); return 0 } if (operation === "set") { const profiles = rest[3]?.split(",").filter(Boolean); if (!role || !profiles?.length) throw new Error("Usage: mathos provider fallback set <role> <profile-id,...>"); for (const profileId of profiles) if (!registry.get(profileId)) throw new Error(`MODEL_PROFILE_NOT_FOUND: ${profileId}`); const prefix = `model.fallback.${role}`; mkdirSync(dirname(configPath), { recursive: true }); writeFileSync(configPath, serializeConfigValues({ ...existing, [`${prefix}.profiles`]: profiles, [`${prefix}.allow_billing_transition`]: rest.includes("--allow-billing-transition"), [`${prefix}.allow_local_to_remote_transition`]: rest.includes("--allow-local-to-remote-transition") }), { encoding: "utf8", mode: 0o600 }); process.stdout.write(`${JSON.stringify({ role, profiles })}\n`); return 0 } throw new Error(`Unknown provider fallback action: ${operation}`) }
      if (!id) throw new Error("MODEL_PROFILE_ID_REQUIRED")
      if (action === "add" || action === "configure") { const legacy = action === "add", descriptorId = legacy ? "generic-openai-compatible" : id, profileId = legacy ? id : flag(rest, "--profile"); if (!profileId) throw new Error("MODEL_PROFILE_ID_REQUIRED"); const descriptor = providerCatalog.get(descriptorId); if (!descriptor) throw new Error(`PROVIDER_DESCRIPTOR_NOT_FOUND: ${descriptorId}`); const baseUrl = flag(rest, "--base-url") ?? null, model = flag(rest, "--model") ?? "auto", protocol = flag(rest, "--protocol") ?? null, generic = descriptor.id.startsWith("generic-"); if (protocol && !["openai-chat", "openai-responses", "anthropic-messages"].includes(protocol)) throw new Error("PROVIDER_PROTOCOL_INVALID"); if (protocol && !generic && !descriptor.modelProtocols) throw new Error("PROVIDER_PROTOCOL_OVERRIDE_FORBIDDEN"); const extraHeaders = Object.fromEntries(rest.flatMap((value, index) => rest[index - 1] === "--header" ? [value] : []).map(value => { const at = value.indexOf(":"); if (at < 1) throw new Error("MODEL_PROFILE_HEADER_INVALID"); return [value.slice(0, at).trim(), value.slice(at + 1).trim()] })), sessionHeader = flag(rest, "--session-header") ?? null; if (legacy && (!baseUrl || model === "auto")) throw new Error("MODEL_PROFILE_ARGUMENTS_INVALID"); if (baseUrl && !descriptor.id.startsWith("generic-")) throw new Error("PROVIDER_BASE_URL_OVERRIDE_FORBIDDEN"); const now = new Date().toISOString(), auth: ModelProfileAuth = legacy && rest.includes("--local") ? { kind: "none" } : descriptor.authKinds.includes("upstream-client") ? { kind: "upstream-client", accountAlias: flag(rest, "--account-alias") ?? null, clientHome: null } : descriptor.authKinds.includes("copilot-logged-in-user") ? { kind: "copilot-logged-in-user" } : descriptor.authKinds.includes("application-default") ? { kind: "application-default", projectId: null, location: null } : descriptor.authKinds.includes("secret-ref") ? { kind: "secret-ref", secretRef: flag(rest, "--secret-ref") ?? `model.${profileId}` } : { kind: "none" }; const roles = (flag(rest, "--roles")?.split(",").filter(Boolean) ?? ["primary","auditor","alignment","planner","repair","intake","formalizer","prover","checker","literature_synthesis","researcher"]) as ModelRole[]; const profile: ModelProfileV2 = { schemaVersion: "mathos.model-profile.v2", id: profileId, descriptorId, displayName: profileId, model, endpointPresetId: protocol ?? flag(rest, "--endpoint-preset") ?? (generic ? descriptor.transport : descriptor.endpointPresets[0]?.id ?? null), baseUrlOverride: baseUrl, auth, enabled: true, timeoutMs: Number(flag(rest, "--timeout-ms") ?? 60_000), maxResponseBytes: 2_000_000, maxOutputTokens: flag(rest, "--max-output-tokens") ? Number(flag(rest, "--max-output-tokens")) : null, reasoningEffort: (flag(rest, "--reasoning") as ModelProfileV2["reasoningEffort"]) ?? null, allowedRoles: roles, requestConcurrency: 1, ...(Object.keys(extraHeaders).length ? { extraHeaders } : {}), ...(sessionHeader ? { sessionHeader } : {}), metadata: { createdAt: now, updatedAt: now, migratedFromV1: false } }; const existing = registry.get(profileId); if (rest.includes("--update")) { if (!existing) throw new Error(`MODEL_PROFILE_NOT_FOUND: ${profileId}`); if (existing.descriptorId !== descriptorId) throw new Error("MODEL_PROFILE_DESCRIPTOR_MISMATCH"); registry.update({ ...profile, auth: existing.auth.kind === profile.auth.kind ? existing.auth : profile.auth, displayName: existing.displayName, allowedRoles: flag(rest, "--roles") ? profile.allowedRoles : existing.allowedRoles, enabled: existing.enabled, metadata: { ...existing.metadata, updatedAt: now } }) } else registry.add(profile); save(); process.stdout.write(`${JSON.stringify({ configured: true, profile: registry.get(profileId), warning: legacy ? "DEPRECATED: use provider configure generic-openai-compatible --profile <id>" : null }, null, 2)}\n`); return 0 }
      ...
      if (action === "login" || action === "install-client") {
        const profile = registry.get(id ?? ""); if (!profile) throw new Error(`MODEL_PROFILE_NOT_FOUND: ${id}`)
        if (!["upstream-client","copilot-logged-in-user"].includes(profile.auth.kind)) throw new Error("PROVIDER_LOGIN_NOT_SUPPORTED")
        const descriptor = providerCatalog.get(profile.descriptorId), policy = evaluateProviderPolicy(profile.descriptorId)
        if (!policy.allowed) throw new Error(policy.code)
        const spec = descriptor ? clientLoginFor(descriptor) : null; if (!spec) throw new Error("PROVIDER_LOGIN_NOT_SUPPORTED")
        const runtime = bunClientRuntime()
        if (action === "install-client") { const result = await openClientInstaller(runtime, spec); process.stdout.write(json ? `${JSON.stringify({ profile: profile.id, client: spec.client, ...result })}\n` : `${result.opened ? "Opened the official installer in a new window" : "Run the official installer"}: ${result.command}\n${result.url}\n`); return 0 }
        // In a terminal, run the client's own sign-in right here; the desktop asks for --background (or --window).
        if (!json && !rest.includes("--background") && !rest.includes("--window") && !rest.includes("--check")) {
          const argv = clientArgv(runtime, spec, spec.loginArgs)
          if (!argv) { process.stdout.write(`${spec.displayName} is not installed. Install it with:\n  ${process.platform === "win32" ? spec.install.windows : spec.install.unix}\n${spec.install.url}\n`); return 1 }
          return await Bun.spawn(argv, { stdio: ["inherit", "inherit", "inherit"], env: { ...process.env, PATH: runtime.env.PATH } }).exited
        }
        const result = await startClientLogin(runtime, spec, { window: rest.includes("--window"), check: rest.includes("--check") })
        process.stdout.write(`${JSON.stringify({ profile: profile.id, clientName: spec.displayName, ...result })}\n`); return 0
      }
      ...
      if (action === "test") { const profile = registry.get(id); if (!profile) throw new Error(`MODEL_PROFILE_NOT_FOUND: ${id}`); const descriptor = providerCatalog.get(profile.descriptorId)!; const policy = evaluateProviderPolicy(descriptor.id); if (!policy.allowed) throw new Error(policy.code); if (rest.includes("--live") && descriptor.billingClass === "payg" && !rest.includes("--accept-usage")) throw new Error("LIVE_USAGE_ACCEPTANCE_REQUIRED"); const report=await runProviderLiveSmoke([id,...rest.slice(2).filter(value=>value.startsWith("--"))],{profiles:registry.list()}); process.stdout.write(`${JSON.stringify(report,null,2)}\n`); return report.connection==="ERROR"||report.liveRequest==="INVALID_STRUCTURED_RESPONSE"?2:0 }
    ...
    const modelRoutes = command && MODEL_COMMANDS.has(command)
