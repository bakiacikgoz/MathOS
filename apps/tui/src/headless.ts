...
export async function runHeadless(argv: string[]): Promise<number> {
  ...
  try {
    ...
    if (command === "provider") {
      ...
      if (action === "list") { process.stdout.write(`${JSON.stringify({ schemaVersion: "mathos.providers.list.v1", profiles: registry.list() }, null, 2)}\n`); return 0 }
      if (action === "status") { const profiles = id ? [registry.get(id)].filter(Boolean) as ModelProfileV2[] : registry.list(); const rows = profiles.map(profile => { const descriptor = providerCatalog.get(profile.descriptorId); if (!descriptor) throw new Error(`PROVIDER_DESCRIPTOR_NOT_FOUND: ${profile.descriptorId}`); const policy = evaluateProviderPolicy(descriptor.id); return { profile: profile.id, descriptor: descriptor.id, connection: !profile.enabled ? "BLOCKED" : !policy.allowed ? policy.code : profile.auth.kind === "secret-ref" ? "SECRET_REQUIRED" : profile.auth.kind === "upstream-client" || profile.auth.kind === "copilot-logged-in-user" ? "LOGIN_REQUIRED" : descriptor.remote ? "CONFIGURED" : "LOCAL_OFFLINE", model: profile.model, billing: descriptor.billingClass, terms: policy.code, auth: profile.auth.kind } }); if (id && !rows.length) throw new Error(`MODEL_PROFILE_NOT_FOUND: ${id}`); process.stdout.write(`${json ? JSON.stringify({ schemaVersion: "mathos.providers.status.v1", profiles: rows }, null, 2) : rows.map(formatProviderStatus).join("\n\n")}\n`); return 0 }
      ...
      if (action === "login") { const profile = registry.get(id); if (!profile) throw new Error(`MODEL_PROFILE_NOT_FOUND: ${id}`); if (!["upstream-client","copilot-logged-in-user"].includes(profile.auth.kind)) throw new Error("PROVIDER_LOGIN_NOT_SUPPORTED"); process.stdout.write(`${JSON.stringify({ delegated: true, profile: id, authOwner: profile.auth.kind, state: "LOGIN_REQUIRED" })}\n`); return 0 }
