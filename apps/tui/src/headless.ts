...
export async function runHeadless(argv: string[]): Promise<number> {
  ...
  try {
    ...
    if (command === "provider") {
      ...
      if (action === "status") { const profiles = id ? [registry.get(id)].filter(Boolean) as ModelProfileV2[] : registry.list(); const secretStore = createSecretStore(), hasSecret = async (ref: string) => { try { return Boolean(await secretStore.get(ref)) } catch { return false } }; const rows = await Promise.all(profiles.map(async profile => { const descriptor = providerCatalog.get(profile.descriptorId); if (!descriptor) throw new Error(`PROVIDER_DESCRIPTOR_NOT_FOUND: ${profile.descriptorId}`); const policy = evaluateProviderPolicy(descriptor.id); return { profile: profile.id, descriptor: descriptor.id, connection: !profile.enabled ? "BLOCKED" : !policy.allowed ? policy.code : profile.auth.kind === "secret-ref" ? (await hasSecret(profile.auth.secretRef) ? "CONFIGURED" : "SECRET_REQUIRED") : profile.auth.kind === "upstream-client" || profile.auth.kind === "copilot-logged-in-user" ? "LOGIN_REQUIRED" : descriptor.remote ? "CONFIGURED" : "LOCAL_OFFLINE", model: profile.model, billing: descriptor.billingClass, terms: policy.code, auth: profile.auth.kind } })); if (id && !rows.length) throw new Error(`MODEL_PROFILE_NOT_FOUND: ${id}`); process.stdout.write(`${json ? JSON.stringify({ schemaVersion: "mathos.providers.status.v1", profiles: rows }, null, 2) : rows.map(formatProviderStatus).join("\n\n")}\n`); return 0 }
