...
export function requiredModelRoles(command: string, configuredRoles: Record<string, string>): ModelRole[] {
  ...
  if (command === "align") return [configuredRoles.alignment ? "alignment" : "auditor"]
...
export async function runHeadless(argv: string[]): Promise<number> {
  ...
  try {
    ...
    if (command === "provider") {
      ...
      const json = rest.includes("--json")
      // The same privacy rule real use applies (model-runtime): remote models need privacy.allow_remote_models.
