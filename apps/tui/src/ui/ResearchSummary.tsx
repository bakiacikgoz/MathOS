...
export function ResearchSummary(props: { status: StatusProjection; home?: string; compact?: boolean; dashboardWidth?: number; dashboardHeight?: number; naturalStatement?: string; formalText?: string | null; run?: ResearchRun | null; steps?: ResearchStep[] }) {
  const objective = () => props.status.mainObjective
  // Progress must come from a recorded step; status lines such as "0 verified" are counts, not progress.
  const lastSucceeded = () => props.steps?.slice().reverse().find((step) => step.status === "SUCCEEDED") ?? null
  const meaningful = () => lastSucceeded()?.summary ?? lastSucceeded()?.action ?? "No recorded progress"
  const [measuredWidth, setMeasuredWidth] = createSignal(props.dashboardWidth ?? 100)
  const [measuredHeight, setMeasuredHeight] = createSignal(props.dashboardHeight ?? FULL_DASHBOARD_ROWS)
  // Fixed sections: objective 5, summary 10, progress 4, plus one-row gaps; the command grid needs 12 rows and a gap.
  const showCommands = () => measuredHeight() >= FULL_DASHBOARD_ROWS
  const activityRows = () => Math.max(1, measuredHeight() - 22 - (showCommands() ? 13 : 0) - 3)
  const activities = () => (props.steps ?? []).slice(-activityRows()).reverse()
  ...
  return (
    ...
    <box flexDirection="column" paddingLeft={1} paddingRight={1} flexGrow={1} gap={1} onSizeChange={function () { setMeasuredWidth(this.width); setMeasuredHeight(this.height) }}>
      ...
      <box height={4} flexShrink={0} flexDirection="column" border borderColor={theme.border}>
        <Title text="LATEST MEANINGFUL PROGRESS" color={theme.violet} /><Show when={lastSucceeded()} fallback={<text fg={theme.textMuted}>{" No recorded progress yet"}</text>}><text fg={theme.success}>{` ✓  ${clock(lastSucceeded()?.finishedAt ?? lastSucceeded()?.startedAt)}  ${meaningful()}`}</text></Show>
      </box>

      <box minHeight={4} flexGrow={1} flexDirection="column" border borderColor={theme.border} overflow="hidden">
        <Title text="RECENT ACTIVITY" color={theme.violet} />
        <Show when={activities().length} fallback={<text fg={theme.textMuted}>{" No research activity yet. Run /research to start one."}</text>}>
          <For each={activities()}>{(item) => <Activity step={item} />}</For>
        </Show>
      </box>
...
function CompactDashboard(props: { status: StatusProjection; formalText?: string | null; meaningful: string; dashboardWidth?: number }) {
  ...
  return <box flexDirection="column" paddingLeft={1} paddingRight={1} flexGrow={1}>
    <box height={3} flexShrink={0} flexDirection="row" alignItems="center" border borderColor={theme.border} paddingLeft={1}><text fg={theme.blue}>OBJECTIVE</text><text fg={theme.accent}>{`  ${objective()?.id ?? "—"}  `}<span style={{ fg: theme.text }}>{objective()?.title ?? "No main claim"}</span></text></box>
    <box height={4} flexShrink={0} flexDirection="column" border borderColor={theme.border}><Title text="RESEARCH SUMMARY" color={theme.blue} /><text fg={theme.textMuted}>{"  Status "}<span style={{ fg: statusColor(objective()?.status ?? "OPEN") }}>{objective()?.status ?? "OPEN"}</span>{`  ·  Verified ${props.status.research.verified}/${props.status.research.totalClaims}  ·  Blockers ${props.status.research.blocked}`}</text></box>
    <box height={3} flexShrink={0} flexDirection="row" alignItems="center" border borderColor={theme.border} paddingLeft={1}><text fg={theme.violet}>LATEST PROGRESS</text><text fg={theme.success}>{`  ${props.meaningful}`}</text></box>
...
function Metric(props: { label: string; value: string; color?: string }) { return <box height={1} flexShrink={0} flexDirection="row" paddingLeft={1} paddingRight={1} overflow="hidden"><text width={18} flexShrink={0} fg={theme.textMuted}>{props.label}</text><text fg={props.color ?? theme.text}>{props.value}</text></box> }
function Activity(props: { step: ResearchStep }) { return <box height={1} flexShrink={0} overflow="hidden" flexDirection="row" paddingLeft={1} paddingRight={1}><text width={10} fg={theme.textMuted}>{clock(props.step.finishedAt ?? props.step.startedAt)}</text><text flexGrow={1} fg={props.step.status === "SUCCEEDED" ? theme.text : theme.warning}>{props.step.summary ?? props.step.action}</text><text fg={theme.textMuted}>{`[${props.step.action}]`}</text></box> }
