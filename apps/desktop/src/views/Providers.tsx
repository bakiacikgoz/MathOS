...
import { acceptsProtocol, configureArgs, groupOf, isGeneric, providerKeys, secretEnvName, suggestProfileId, useCatalog, useProfiles, useProviderStatus, usesUpstreamLogin, validProfileId, type CatalogEntry, type ProviderDescriptor, type ProviderGroup, type StatusRow, type WireProtocol } from "../lib/providers.ts"
...
interface WizardState { descriptor: ProviderDescriptor; step: WizardStep; profile: string | null }
...
export function Providers() {
  ...
  return (
    <div className="page-inner providers-page">
              ...
              <div key={row.profile} className="profile-row" style={{ "--i": index } as React.CSSProperties}>
                ...
                <div className="actions">
                  {row.connection === "SECRET_REQUIRED" ? <button className="btn btn-primary btn-sm" onClick={() => openFor(row, "key")}><Icon name="plus" size={14} />{t("providers.addKey")}</button>
                    : row.connection === "LOGIN_REQUIRED" ? <button className="btn btn-secondary btn-sm" onClick={() => openFor(row, "key")}>{t("providers.howToLogin")}</button>
      ...
      <ConnectWizard state={wizard} taken={(profiles.data?.profiles ?? []).map((profile) => profile.id)} hasDefault={Boolean(defaultProfile)}
...
function ConnectWizard({ state, taken, hasDefault, secretRefOf, onStep, onClose }: { state: WizardState | null; taken: string[]; hasDefault: boolean; secretRefOf: (id: string) => string; onStep: (step: WizardStep, profile?: string) => void; onClose: () => void }) {
  const app = useApp()
  const { t, lang } = useT()
  ...
  const identity = state ? `${state.descriptor.id}|${state.profile ?? ""}` : null
  if (descriptor && identity !== forKey) {
    ...
    setForm({ profile: state?.profile ?? suggestProfileId(descriptor, taken), model: descriptor.defaultModels[0] ?? "", baseUrl: "", protocol: isGeneric(descriptor) ? descriptor.transport as WireProtocol : "", headers: "" })
  }
  ...
  const footer = state.step === "setup" ? <>
      ...
      {upstream && <button className="btn btn-primary" onClick={() => onStep("test")}>{t("providers.continue")}</button>}
    </> : <>
  ...
  return (
    <Sheet open onClose={onClose} title={providerName(descriptor, lang)} footer={footer}>
      ...
      <ol className="stepper" aria-label={t("providers.progress")}>
        {steps.map((step, index) => { const current = steps.findIndex((item) => item.id === state.step); return <li key={step.id} className={index < current ? "done" : index === current ? "current" : ""} aria-current={index === current ? "step" : undefined}><span className="n">{index < current ? <Icon name="check" size={11} stroke={2.6} /> : index + 1}</span>{t(step.label)}</li> })}
      </ol>
      ...
      {state.step === "setup" && <form onSubmit={(event) => { event.preventDefault(); void saveSetup() }} style={{ display: "contents" }}>
        ...
        <label className="field"><span className="field-label">{t("providers.model")}</span>
          <input className="input" value={form.model} onChange={set("model")} placeholder={generic ? "model-id" : "auto"} spellCheck={false} disabled={Boolean(state.profile)} />
          {touched && errors.model && <span className="field-error">{t("common.required")}</span>}
          {!state.profile && descriptor.defaultModels.length > 0 && <div className="chips">{descriptor.defaultModels.slice(0, 8).map((model) => <button key={model} type="button" className={`chip ${form.model === model ? "on" : ""}`} onClick={() => setForm((value) => ({ ...value, model }))}>{model}</button>)}</div>}
        </label>
        ...
        {!state.profile && <button type="button" className="link-btn" onClick={() => setAdvanced((value) => !value)} aria-expanded={advanced}>{t("providers.advanced")} <Icon name="chevrons" size={12} /></button>}
        {advanced && !state.profile && <div className="advanced">
      ...
      {state.step === "key" && upstream && <div className="key-step">
        <div className="n"><Icon name="console" size={13} /></div>
        <div><strong>{t("providers.loginTitle")}</strong><p className="field-hint">{t("providers.upstreamHint")}</p>
          <code className="command selectable">mathos provider login {profileId}</code></div>
      </div>}
