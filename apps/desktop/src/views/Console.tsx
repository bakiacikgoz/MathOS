...
export function Console() {
  ...
  return (
    <div className="console">
      ...
      <div className="console-out" ref={out}>
        {entries.length === 0 && (
          <div className="view-enter" style={{ fontFamily: "var(--font)" }}>
            ...
            <div className="examples">{EXAMPLES.map((example) => <button key={example} className="chip" onClick={() => submit(example)}>{example}</button>)}</div>
          </div>
        )}
        {entries.map((entry) => (
      ...
      <form className="console-input" onSubmit={(event) => { event.preventDefault(); void submit(value) }}>
