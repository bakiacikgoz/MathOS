...
export class AlignmentService{
  ...
  recordExplicitHumanApproval(claimId:string,actorId:string):FormalAlignment{
    const natural=this.d.revisions.latest(claimId,"NATURAL"),formal=this.d.revisions.latest(claimId,"FORMAL");if(!natural||!formal||natural.contextRevisionId!==formal.contextRevisionId)throw new Error("STATEMENT_REVISIONS_REQUIRED")
    const existing=this.d.alignments.latestForClaim(claimId);if(existing&&alignmentAuthorizesVerification(existing,{naturalRevisionId:natural.id,formalRevisionId:formal.id,contextRevisionId:natural.contextRevisionId}))return existing
    const now=this.d.clock.now(),alignment:FormalAlignment={id:this.d.nextId("AL"),claimId,naturalRevisionId:natural.id,formalRevisionId:formal.id,contextRevisionId:natural.contextRevisionId,status:"HUMAN_APPROVED",verdict:"MATCH",backTranslation:"Explicit legacy fidelity approval bound to exact statement revisions.",symbolMapping:[],auditorProvider:null,auditorModel:null,promptHash:null,createdAt:now,decidedAt:now};this.d.alignments.insert(alignment);this.d.writeEvent?.("alignment.human-approved",{alignmentId:alignment.id,actorId,naturalHash:natural.contentHash,formalHash:formal.contentHash,contextRevisionId:natural.contextRevisionId});return alignment
  }
}
