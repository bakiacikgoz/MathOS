...
export function promotionReport(paired: PairedAnalysis, downstream: PromotionReport["downstream"], environmentReady = true): PromotionReport {
  const checks: PromotionReport["checks"] = {
    ...
    downstreamNonDecreasing: downstream.candidate.rate >= downstream.baseline.rate,
    environmentReady: environmentReady ? true : null,
  }
  const reasons = Object.entries(checks).filter(([, value]) => value !== true).map(([key, value]) => `${key}:${value === null ? "unavailable" : "failed"}`)
  const decision: PromotionDecision = !environmentReady || paired.baseline.aggregate.cases === 0 || downstream.baseline.attempted === 0 ? "INCONCLUSIVE" : reasons.length ? "REJECT" : "PROMOTE"
