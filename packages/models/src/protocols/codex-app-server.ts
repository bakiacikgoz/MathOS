...
export class CodexAppServerClient{
 ...
 constructor(executable:string,args:string[]=["app-server"],options:{env?:NodeJS.ProcessEnv;schema:CodexSchema;version:string;timeoutMs?:number}){validateCodexSchema(options.schema);this.scratch=mkdtempSync(join(tmpdir(),"mathos-codex-"));this.rpc=new JsonRpcProcess({executable,args,cwd:this.scratch,env:options.env,onNotification:(method,params)=>this.notification(method,params),documentedAuthEnv:[],omitVersionHeader:true});this.version=options.version;this.timeoutMs=options.timeoutMs??60_000}
 private readonly version:string;private readonly timeoutMs:number
 ...
 async infer(input:{model:string;messages:Array<{role:string;content:string}>;signal?:AbortSignal}){
  ...
  try{
   ...
   if(!this.messages.length&&!this.protocolViolation)await Promise.race([done,new Promise<never>((_,reject)=>setTimeout(()=>{timedOut=true;reject(new Error("CODEX_TURN_TIMEOUT"))},this.timeoutMs))])
