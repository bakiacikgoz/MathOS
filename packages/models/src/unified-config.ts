...
export interface MathOSConfig {
  model: { default_profile: string; roles: Record<string, string> }
...
const DEFAULTS: MathOSConfig = {
  model: { default_profile: "", roles: {} }, literature: { providers: ["openalex", "crossref", "arxiv"], offline: false },
...
export function parseMathOSConfig(text: string): Record<string, ConfigScalar> {
  ...
  for (const rawLine of text.split(/\r?\n/)) {
    ...
    if (!FIXED.has(key) && !key.startsWith("model.roles.")) throw new Error(`CONFIG_UNKNOWN_KEY: ${key}`)
...
export function loadMathOSConfig(options: { userToml?: string; workspaceToml?: string; env?: NodeJS.ProcessEnv; cli?: Record<string, ConfigScalar> } = {}): LoadedMathOSConfig {
  ...
  const apply = (values: Record<string, ConfigScalar>, source: LoadedMathOSConfig["sources"][string]) => { for (const [key, value] of Object.entries(values)) { if (!FIXED.has(key) && !key.startsWith("model.roles.")) throw new Error(`CONFIG_UNKNOWN_KEY: ${key}`); assign(config, key, value); sources[key] = source } }
...
export function serializeWorkspaceConfig(path: string, value: ConfigScalar): string {
  if (SECRET.test(path)) throw new Error(`CONFIG_SECRET_FORBIDDEN: ${path}`)
  if (!FIXED.has(path) && !path.startsWith("model.roles.")) throw new Error(`CONFIG_UNKNOWN_KEY: ${path}`)
