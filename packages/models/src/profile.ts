...
import { assertSafeProviderUrl } from "./redact.ts"
...
function validateOverride(profile: ModelProfileV2): string | null { if (!profile.baseUrlOverride) return null; let url:URL;try{url=assertSafeProviderUrl(profile.baseUrlOverride,{allowLoopback:LOCAL_DESCRIPTORS.has(profile.descriptorId)||(profile.descriptorId.startsWith("generic-")&&profile.auth.kind==="none")})}catch(error){if(error instanceof Error&&error.message==="PROVIDER_URL_INVALID")throw new Error("MODEL_PROFILE_URL_INVALID");if(error instanceof Error&&error.message==="PROVIDER_URL_UNSAFE")throw new Error("MODEL_PROFILE_URL_UNSAFE");throw error}if(LOCAL_DESCRIPTORS.has(profile.descriptorId)&&!["localhost","127.0.0.1","::1"].includes(url.hostname))throw new Error("LOCAL_MODEL_MUST_USE_LOOPBACK");return profile.baseUrlOverride.replace(/\/$/, "") }
const HEADER_NAME = /^[A-Za-z0-9!#$%&'*+.^_`|~-]{1,64}$/
const CREDENTIAL_HEADER = /(auth|api[_-]?key|token|secret|cookie|password|credential)/i
const RESERVED_HEADER = /^(host|content-type|content-length|connection|transfer-encoding|user-agent|anthropic-version)$/i
/** Extra headers are for routing/attribution only; anything that could carry a credential belongs in the secret store. */
export function validateProviderHeaderName(name: string): string { if (!HEADER_NAME.test(name)) throw new Error("MODEL_PROFILE_HEADER_INVALID"); if (CREDENTIAL_HEADER.test(name)) throw new Error("MODEL_PROFILE_SECRET_FORBIDDEN"); if (RESERVED_HEADER.test(name)) throw new Error("MODEL_PROFILE_HEADER_RESERVED"); return name.toLowerCase() }
function validateHeaders(profile: ModelProfileV2): Pick<ModelProfileV2, "extraHeaders" | "sessionHeader"> {
  const custom = profile.extraHeaders && Object.keys(profile.extraHeaders).length ? profile.extraHeaders : null
  if ((custom || profile.sessionHeader) && !profile.descriptorId.startsWith("generic-")) throw new Error("MODEL_PROFILE_HEADERS_GENERIC_ONLY")
  const extraHeaders = custom ? Object.fromEntries(Object.entries(custom).map(([name, value]) => { if (typeof value !== "string" || value.length > 256 || /[\r\n\0]/.test(value)) throw new Error("MODEL_PROFILE_HEADER_INVALID"); return [validateProviderHeaderName(name), value] })) : null
  if (extraHeaders && Object.keys(extraHeaders).length > 16) throw new Error("MODEL_PROFILE_HEADER_INVALID")
  return { extraHeaders, sessionHeader: profile.sessionHeader ? validateProviderHeaderName(profile.sessionHeader) : null }
}
export function validateModelProfileV2(profile: ModelProfileV2): ModelProfileV2 { rejectSecrets(profile); if (profile.schemaVersion !== MODEL_PROFILE_V2_SCHEMA) throw new Error("MODEL_PROFILE_SCHEMA_UNSUPPORTED"); validId(profile.id, "MODEL_PROFILE_ID_INVALID"); validId(profile.descriptorId, "PROVIDER_DESCRIPTOR_ID_INVALID"); if (!profile.displayName.trim()) throw new Error("MODEL_PROFILE_DISPLAY_NAME_REQUIRED"); if (!profile.model.trim()) throw new Error("MODEL_PROFILE_MODEL_REQUIRED"); if (profile.auth.kind === "secret-ref" && !/^[a-z0-9][a-z0-9._-]{0,127}$/i.test(profile.auth.secretRef)) throw new Error("MODEL_PROFILE_SECRET_REF_INVALID"); if (!Number.isInteger(profile.timeoutMs) || profile.timeoutMs < 100 || !Number.isInteger(profile.maxResponseBytes) || profile.maxResponseBytes < 1 || !Number.isInteger(profile.requestConcurrency) || profile.requestConcurrency < 1) throw new Error("MODEL_PROFILE_LIMIT_INVALID"); if (!profile.allowedRoles.every(role => ROLES.has(role))) throw new Error("MODEL_PROFILE_ROLE_INVALID"); const headers = validateHeaders(profile); return { ...profile, baseUrlOverride: validateOverride(profile), allowedRoles: [...new Set(profile.allowedRoles)], ...(headers.extraHeaders ? { extraHeaders: headers.extraHeaders } : {}), ...(headers.sessionHeader ? { sessionHeader: headers.sessionHeader } : {}) } }
